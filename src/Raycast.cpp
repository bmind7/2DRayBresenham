...
namespace WGPrep
{
		...
		bool updateGrid)
	{
		...
		if (!GridInBounds(grid, x0, y0) || !GridInBounds(grid, x1, y1))
		...
		while (true)
		{
			if (!skipStart && GridAt(grid, x, y) != ECellType::Empty)
			{
				out.IsVisible = false;
				out.HitX = x;
				out.HitY = y;
			...
			if (updateGrid && GridAt(grid, x, y) == ECellType::Empty)
				GridAt(grid, x, y) = ECellType::Ray;
